...
let ascById = document.querySelector(".asc");
let descById = document.querySelector(".desc");
let defaultSuppliers = document.querySelector(".default");
...
ascById.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => a.id - b.id);
  drawTable(sorted);
});
descById.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => b.id - a.id);
  drawTable(sorted);
});
defaultSuppliers.addEventListener("click", function () {
  drawTable(suppliersCopy);
});

sortByName.addEventListener("click", function () {
  let sorted;
  if (this.innerText === "sort by name asc") {
    this.innerText = "sort by name desc";
    sorted = suppliers.sort((a, b) =>
      a.companyName
        .toLocaleLowerCase()
        .localeCompare(b.companyName.toLocaleLowerCase())
    );
  } else if (this.innerText === "sort by name desc") {
    this.innerText = "back to default";
    sorted = suppliers.sort((a, b) =>
      b.companyName
        .toLocaleLowerCase()
        .localeCompare(a.companyName.toLocaleLowerCase())
    );
  } else {
    this.innerText = "sort by name asc";
...
search.addEventListener("keyup", function (event) {
  let filtered = suppliers.filter((element) =>
    element.companyName
      .toLocaleLowerCase()
